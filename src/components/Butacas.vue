<template>
  ...
  <div class="comprar-entradas">
    <p id="total-price">Precio Total: {{ precioTotal }} €</p>
  ...
  <div v-if="detalleCompraVisible" class="detalle-compra">
    ...
    <p><strong>Sesión:</strong> {{ idSesion }}</p>
...
store.asientos.forEach((asiento, index) => {
  ...
  // Agregar el número del id encima de la butaca
  const numeroId = asiento.idAsiento.toString();
  const textX = x + anchoAsiento / 2;
  const textY = y + altoAsiento / 2 - 5; // Ajuste vertical para colocar el número encima de la butaca
